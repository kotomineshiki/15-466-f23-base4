...
Scene::Drawable::Pipeline lit_color_texture_trans_program_pipeline;
...
Load<LitColorTextureProgram> lit_color_texture_program(LoadTagEarly, []() -> LitColorTextureProgram const *
	...
	*/
	...
	//----- build the pipeline template -----
	lit_color_texture_trans_program_pipeline.program = ret->program;

	lit_color_texture_trans_program_pipeline.OBJECT_TO_CLIP_mat4 = ret->OBJECT_TO_CLIP_mat4;
	lit_color_texture_trans_program_pipeline.OBJECT_TO_LIGHT_mat4x3 = ret->OBJECT_TO_LIGHT_mat4x3;
	lit_color_texture_trans_program_pipeline.NORMAL_TO_LIGHT_mat3 = ret->NORMAL_TO_LIGHT_mat3;
	...
	lit_color_texture_trans_program_pipeline.textures[0].texture = tex;
	lit_color_texture_trans_program_pipeline.textures[0].target = GL_TEXTURE_2D;
...
LitColorTextureTransProgram::LitColorTextureTransProgram()
{
	...
	program = gl_compile_program(
		...
		"	}\n"
		"	vec4 albedo = texture(TEX, texCoord) * color;\n"
		"	fragColor = vec4(albedo.rgb, pow(albedo.a,4));\n"
		"}\n");
	// As you can see above, adjacent strings in C/C++ are concatenated.
	//  this is very useful for writing long shader programs inline.
